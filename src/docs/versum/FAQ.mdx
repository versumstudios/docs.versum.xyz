...
Secondly, our user experience reflects our aim to redefine the art experience within the NFT space. It was important for us to combine features of NFT platforms and social platforms to introduce a marketplace that focuses on organic discovery, decentralized storage, and accessibility.
...
There was a lot of thinking around common issues that arise from a blockchain with low gas fees — from copyminter protection to restricting the number of token editions a wallet can have, we offer artists and collectors tools so they are in control.
...
1. Creative Commons Zero v1.0 Universal ([CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/))
2. Creative Commons Attribution 4.0 International ([CC BY 4.0](https://creativecommons.org/licenses/by/4.0/))
3. Attribution-ShareALike 4.0 International ([CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/))
   <br />
   <br />
   You are also able to create a custom license.

## How can tokens be traded?

You can conduct swaps, auctions, and offers.

**Swaps** — There are two types of swaps — fixed price and advanced. Fixed price swaps are always sold at the same price, and advanced swaps have a defined start price and end price. The price will automatically adjust based on how many tokens are left.

**Auctions** — A token goes up for auction with a minimum purchase price (in XTZ or FA2) and a duration (in hours).

**Offers** — When an offer is made on a token any collector who has that token receives a notification asking if they would like to sell this token (in XTZ or FA2).
...
**On-chain verification** is required for users to mint. If the seller/buyer specifies it, users will require this verification to interact with certain swaps, auctions, and offers. This allows us to filter out most botters, scammers, and copyminters.
**Off-chain verification** is when the state of the user gets verified off-chain and the user is provided with a badge
**Unverified users** that aren’t registered with [Tezos Profiles](https://tzprofiles.com/) or [tzkt](https://tzkt.io) will have some restrictions in place.

To note, users registered with Tezos Profiles will be able to add multiple claims to their profile. The more claims you have, the higher your score. Above a certain score you will achieve a verified status.
...
## Why is Versum trading in Tezos?
